<x-app-layout>
    <x-slot name="header">
        ...
        <div>
            <h1 class="text-center font-semibold text-4xl">Editar de categoria</h1>
        </div>
        ...
        @method('PATCH')
            <div class="grid grid-cols-1 md:grid-cols-1 gap-5 md:gap-8 mt-5 mx-7">
                <div class="grid grid-cols-1">
                    ...
                    <input name="name" value="{{$category->name}}" class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="text" required/>
                </div>
                <div class="grid grid-cols-1">
                    <label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">{{ trans('Description') }}:</label>
                    <input name="description" value="{{$category->description}}" class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="text" required/>
                </div>
            </div>
